...
def get_grid_index_from_movie_frame(sub_data, start_frame, end_frame):
    fps = sub_data["meta"]["movie_fps_nominal"]
...
def slice_modalities_by_movie_frame(sub_data, start_frame, end_frame, bands=None):
    t = np.asarray(sub_data["time_grid"])
    idx = get_grid_index_from_movie_frame(sub_data, start_frame, end_frame)

    out_seg = {
        "frame_grid": t[idx],
        "lfp_macro": sub_data["lfp_macro"][idx] if sub_data["lfp_macro"] is not None else None,
        "lfp_micro": sub_data["lfp_micro"][idx] if sub_data["lfp_micro"] is not None else None,
        "eye_gaze": sub_data["eye_gaze"][idx] if sub_data["eye_gaze"] is not None else None,
        "pupil": sub_data["pupil"][idx] if sub_data["pupil"] is not None else None,
        "firing_rate": sub_data["firing_rate"][idx] if sub_data["firing_rate"] is not None else None,
    }
...
def slice_modalities_by_movie_time_sec(sub_data, start_sec, end_sec):
    ...
    return slice_modalities_by_movie_frame(sub_data, start_frame, end_frame)
